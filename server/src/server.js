...
app.post('/event', function(req, res) {
  ...
  jwt.verify(token, secretKey, function(err, d) {
    if(err) {
      ...
      School.findOne({_id: req.body.schoolId}, 'roster events admins', function(err, school) {
        if(err) {
          res.status(500).send({ msg: RESPONSE_MSGS.ERROR_500 })
        }
        else if(school) {
          if(school.roster.indexOf(d.id) > -1) {
            var data = req.body
            if(data.official == true) {
              if(school.admins.map(function(entry) { return entry._user}).indexOf(d.id) > -1) {
                var event = new Event({
                  name: data.name,
          ...
          res.status(404).send({ msg: "Could not find school."})
        }
      })
    }
  })
    ...
    if(err) {
      ...
      jwt.verify(token, secretKey, function(err, data) {
        if(err) {
          ...
          var idx = event.participants.indexOf(data.id)
          if(idx == -1) {
...
app.delete('/events/rsvp/:id', function(req, res) {
  const ERROR_401 = "Uh oh. Something went wrong. Please try logging in again."
  var authorizationLine = req.headers.authorization;
  var token = authorizationLine.slice(7);
  Event.findOne({_id: req.params.id}, "participants", function(err, event) {
